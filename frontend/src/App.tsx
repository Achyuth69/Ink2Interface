...
function usePreviewUrl(code: string) {
  ...
  useEffect(() => {
    ...
    const baseStyle = `<style>body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;margin:0}img{max-width:100%}a{color:#0073b1}*{box-sizing:border-box}</style>`;
    if (!html.includes("<style")) {
      html = html.includes("</head>") ? html.replace("</head>", `${baseStyle}\n</head>`) : baseStyle + html;
    }
...
export default function App() {
  ...
  // Close sidebar on mobile when result arrives
  useEffect(() => {
      ...
      <div className="p-4 flex flex-col gap-4 flex-1 overflow-y-auto">
        ...
        {sourceMode === "screenshot" && (
          <div>
            <div {...getRootProps()}
              ...
              <p className="text-sm text-gray-400">{isDragActive ? "Drop here" : "Drag & drop or tap to upload"}</p>
        ...
        {sourceMode === "html" && (
          <div>
            <div className="mb-2 bg-blue-950/40 border border-blue-800 rounded-lg px-3 py-2 text-[11px] text-blue-300 leading-relaxed">
              <strong>How to get HTML:</strong> F12 → Elements → right-click &lt;html&gt; → Copy outerHTML
            </div>
            <textarea value={sourceHtml} onChange={(e) => setSourceHtml(e.target.value)}
              rows={8} placeholder="<!DOCTYPE html><html>...</html>" spellCheck={false}
            ...
            <div className="flex items-center justify-between mt-1">
              ...
              {sourceHtml.length > 0 && <button onClick={() => setSourceHtml("")} className="text-[10px] text-gray-600 hover:text-red-400">Clear</button>}
            </div>
          </div>
        )}
        ...
        <div>
          <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1.5 flex items-center gap-1">
            <Globe size={11} /> Reference URL <span className="text-gray-600 font-normal normal-case ml-1">(optional)</span>
          </label>
        ...
        <div>
          <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1.5 block">
            Extra Instructions <span className="text-gray-600 font-normal normal-case ml-1">(optional)</span>
          </label>
        ...
        <div>
          <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1.5 block">
            Model Tier <span className="text-gray-600 font-normal normal-case ml-1">(local GPU only)</span>
          </label>
        ...
        {error && <div className="text-red-400 text-xs bg-red-950/40 border border-red-800 rounded-lg px-3 py-2">{error}</div>}
      </div>

      {/* Generate button */}
      <div className="p-4 border-t border-gray-800 flex gap-2">
        <button onClick={reset} title="Reset" className="p-2.5 rounded-lg bg-gray-800 hover:bg-gray-700 text-gray-400 hover:text-white transition-colors">
          <RotateCcw size={16} />
        </button>
    ...
    <div className="h-screen bg-gray-950 flex flex-col overflow-hidden">
      ...
      <header className="border-b border-gray-800 px-4 py-3 flex items-center justify-between flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-gray-500 mt-0.5 hidden sm:block">Screenshot → HTML + CSS</p>
          </div>
        </div>
        <div className="flex items-center gap-2 text-xs text-gray-500">
          <span className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse inline-block" />
          <span className="hidden sm:inline">Powered by Groq</span>
        </div>
      </header>
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <aside className={`
          ...
          md:relative md:translate-x-0 md:w-[360px] md:flex-shrink-0
          fixed top-[57px] left-0 bottom-0 w-[320px] z-30
        ...
        <main className="flex-1 flex flex-col overflow-hidden bg-gray-950 min-w-0">
          {result ? (
            <>
              ...
              {/* Toolbar */}
              <div className="flex items-center justify-between border-b border-gray-800 px-3 py-2 bg-gray-900/50 flex-shrink-0 gap-2 flex-wrap">
                <div className="flex gap-1 items-center">
                  {/* Mobile: show burger to reopen sidebar */}
                  <button
                    className="md:hidden p-1.5 rounded-lg bg-gray-800 hover:bg-gray-700 text-gray-400 mr-1"
                ...
                <div className="flex items-center gap-1.5 flex-wrap">
                  ...
                  {tab === "preview" && (
                    <button onClick={() => { if (iframeRef.current && previewUrl) { iframeRef.current.src = ""; iframeRef.current.src = previewUrl; } }}
                      className="p-1.5 text-xs bg-gray-800 hover:bg-gray-700 rounded-lg transition-colors text-gray-300" title="Reload">
                      <RefreshCw size={12} />
                    </button>
                  )}
                  <button onClick={downloadCode} className="flex items-center gap-1 text-xs bg-gray-800 hover:bg-gray-700 px-2 py-1.5 rounded-lg transition-colors text-gray-300">
                    <Download size={12} /> <span className="hidden sm:inline">Download</span>
                  </button>
                  <button onClick={downloadSplit} className="flex items-center gap-1 text-xs bg-gray-800 hover:bg-gray-700 px-2 py-1.5 rounded-lg transition-colors text-gray-300">
                    <FileCode size={12} /> <span className="hidden sm:inline">HTML+CSS</span>
                  </button>
                  <button onClick={copyCode} className="flex items-center gap-1 text-xs bg-gray-800 hover:bg-gray-700 px-2 py-1.5 rounded-lg transition-colors text-gray-300">
                    {copied ? <><Check size={12} className="text-emerald-400" /> <span className="hidden sm:inline">Copied!</span></> : <><Copy size={12} /> <span className="hidden sm:inline">Copy</span></>}
                  </button>
                </div>
              </div>
              ...
              {tab === "code" ? (
                <div className="flex-1 overflow-auto">
                  <SyntaxHighlighter language="html" style={vscDarkPlus}
                    customStyle={{ margin: 0, minHeight: "100%", fontSize: "12px", background: "#030712" }}
                    showLineNumbers lineNumberStyle={{ color: "#374151", minWidth: "2.5em" }}>
                    {result.code || " "}
                  </SyntaxHighlighter>
                </div>
              ) : (
                <iframe ref={iframeRef} key={previewUrl} src={previewUrl}
                  className="flex-1 w-full bg-white" title="Preview"
                  sandbox="allow-scripts allow-same-origin allow-forms allow-popups allow-modals" />
              )}
            </>
          ) : (
            <div className="flex-1 flex flex-col items-center justify-center gap-5 text-gray-700 select-none px-4">
